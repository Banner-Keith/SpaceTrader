...
namespace SpaceTrader
{
    public class Ship : ShipSpec
    {
        ...
        {
            return Weapons.Where(t => t != null && t.Type >= min && t.Type <= max).Sum(t => t.Power);
        }
        ...
        public bool ArtifactOnBoard => CommandersShip && Game.CurrentGame.QuestStatusArtifact == SpecialEvent.StatusArtifactOnBoard;
        ...
        public override int CargoBays
        {
            get
            {
                int bays = base.CargoBays + Gadgets.Where(t => t != null && (t.Type == GadgetType.ExtraCargoBays || t.Type == GadgetType.HiddenCargoBays)).Sum(t => 5);
        ...
        public bool CommandersShip => this == Game.CurrentGame.Commander.Ship;
        ...
        public bool DetectableIllegalCargoOrPassengers => DetectableIllegalCargo || IllegalSpecialCargo;

        public bool Disableable => !CommandersShip && Type != ShipType.Bottle && Type != ShipType.Mantis &&
                                   Type != ShipType.SpaceMonster;

        public int Engineer => Skills[(int)SkillType.Engineer];
        ...
        public int ExtraCargoBays
        {
            get
            {
                return Gadgets.Where(t => t != null && t.Type == GadgetType.ExtraCargoBays).Sum(t => 5);
            }
        }

        public int Fighter => Skills[(int)SkillType.Fighter];
        ...
        public int FilledNormalCargoBays => _cargo.Sum();

        public int FreeCargoBays => CargoBays - FilledCargoBays;
        ...
        public int FreeSlots => FreeSlotsGadget + FreeSlotsShield + FreeSlotsWeapon;

        public int FreeSlotsGadget
        {
            get
            {
                return Gadgets.Count(t => t == null);
            }
        }
        ...
        public override int FuelTanks => base.FuelTanks + (HasGadget(GadgetType.FuelCompactor) ? Consts.FuelCompactorTanks : 0);

        public Gadget[] Gadgets => _gadgets;

        public bool HagglingComputerOnBoard => CommandersShip && Game.CurrentGame.QuestStatusJarek == SpecialEvent.StatusJarekDone;

        public int HiddenCargoBays
        {
            get
            {
                return Gadgets.Where(t => t != null && t.Type == GadgetType.HiddenCargoBays).Sum(t => 5);
            }
        }
        ...
        public string HullText => Functions.StringVars(Strings.EncounterHullStrength,
            Functions.FormatNumber((int)Math.Floor((double)100 * Hull / HullStrength)));

        public bool IllegalSpecialCargo => WildOnBoard || ReactorOnBoard || SculptureOnBoard;

        public bool JarekOnBoard => HasCrew(CrewMemberId.Jarek);

        public int Pilot => Skills[(int)SkillType.Pilot];

        public bool PrincessOnBoard => HasCrew(CrewMemberId.Princess);
        ...
        public bool SculptureOnBoard => CommandersShip && Game.CurrentGame.QuestStatusSculpture == SpecialEvent.StatusSculptureInTransit;

        public int ShieldCharge
        {
            get
            {
                return Shields.Where(t => t != null).Sum(t => t.Charge);
            }
        }

        public Shield[] Shields => _shields;
        ...
        public string ShieldText => (Shields.Length > 0 && Shields[0] != null) ?
        ...
        public int[] Skills
        {
            get
            {
                ...
                for (int skill = 0; skill < skills.Length; skill++)
                {
                    ...
                    foreach (CrewMember t in Crew)
                    {
                        if (t != null && t.Skills[skill] > max)
                            max = t.Skills[skill];
                    }
                    ...
                    skills[skill] = Math.Max(1, skill1);
                }

                // Adjust skills based on any gadgets on board.
                foreach (Gadget t in Gadgets)
                {
                    if (t != null && t.SkillBonus != SkillType.Na)
                        skills[(int)t.SkillBonus] += Consts.SkillBonus;
                }
        ...
        public CrewMember[] SpecialCrew
        {
            get
            {
                ...
                foreach (CrewMember t in Crew)
                {
                    if (t != null && Consts.SpecialCrewMemberIds.Contains(t.Id))
                        list.Add(t);
                }
        ...
        public bool[] TradeableItems => _tradeableItems;

        public int Trader => Skills[(int)SkillType.Trader] + (HagglingComputerOnBoard ? 1 : 0);
        ...
        public bool WildOnBoard => HasCrew(CrewMemberId.Wild);
